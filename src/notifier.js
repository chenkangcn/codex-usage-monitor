...
export function alertPresentation(level, event, milestone, windowKind = 'fiveHour') {
  const remaining = Math.max(0, 100 - event.usedPercent);
  const isWeekly = windowKind === 'weekly';
  const titleZh = level === 'critical'
    ? `Codex ${isWeekly ? '周' : '五小时'}额度紧急警告`
    : level === 'severe'
      ? `Codex ${isWeekly ? '周' : '五小时'}额度严重不足`
      : `Codex ${isWeekly ? '周' : '五小时'}用量提醒`;
  const titleEn = level === 'critical'
    ? `Codex ${isWeekly ? 'Weekly' : 'Five-Hour'} Allowance Critical`
    : level === 'severe'
      ? `Codex ${isWeekly ? 'Weekly' : 'Five-Hour'} Allowance Low`
      : `Codex ${isWeekly ? 'Weekly' : 'Five-Hour'} Usage`;
  ...
  const durationSeconds = milestone >= 95 || level === 'critical'
    ? 0
    : level === 'severe'
      ? 20
      : 10;
...
export function createMacNotifier(config) {
  ...
  return async (level, event, milestone, windowKind = 'fiveHour') => {
    const presentation = alertPresentation(level, event, milestone, windowKind);
    try {
